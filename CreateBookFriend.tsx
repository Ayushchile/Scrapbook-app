...
function Card({
  ...
  return (
    <Animated.View style={[styles.card, stylez]}>
      ...
      {index === 0 && (
        <Image
          source={require('./assets/Cover-Page.png')}
...
export default function App()
  ...
  const [images1, setImages] = useState<string[]>([]);
  //const [activeImageBoxes, setActiveImageBoxes] = useState<number[]>([]);
  const [textInputs1, setTextInputs] = useState<string[]>([]);
  ...
  const storeData = async () => {
    try {
      ...
      await AsyncStorage.setItem(
        'selectedData',
        JSON.stringify({images1, textInputs1}),
      );
    } catch (error) {
  ...
  const retrieveData = async () => {
    try {
      ...
      if (storedData !== null) {
        const {images1: storedImages1, textInputs1: storedTextInputs1} =
          JSON.parse(storedData);
        setImages(storedImages1);
        setTextInputs(storedTextInputs1);
      }
    } catch (error) {
              ...
              return (
                <>
                  <Card
                    ...
                    textValue={textInputs1[index] || ''} // Pass text value
                  />
                </>
              );
            })}
